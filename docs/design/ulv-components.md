...
> 状态：**未定案（草案 v0.2，2026-09-07）** · 优先级：**P0（最高优先级，项目所有者 2026-09-07 指定）** · 类型：构件物品 + 覆盖板（cover）

## 定位

上游全部电动构件——电动马达、传送带模块、电动泵、机械臂、发射器、传感器、流体调节器——最低注册于 LV（调研 F12），ULV 层没有任何电动构件：**微电网连一根传送带都接不起来**。本线补齐 ULV 层的基础构件。

- 超低压电动马达：本模组全部 ULV 机器的合成前置，内部需求闭环的核心件。
...
1. 传送带 2 件/t 与泵 16 mB/t 的实机体感；只允许 ±50% 内调参，不得改变「ULV = LV 一半」的公式关系。
2. 上游 cover 渲染器能否按 tier 复用（贴图/色调区分 ULV）？不可复用时的最简渲染方案定案。
3. 马达是否也作为发电机（温差/红石/蒸汽轮机）的合成前置？倾向：**不**——发电机公共前置保持猫须探测器（电子/整流语义），马达专属用电机器（机械传动语义），两条前置线语义分明。
4. 马达是否会与未来可能的「ULV 机器 II 型」等扩展冲突？预留：马达常量容量/耐久不存在，纯材料件，无冲突面。
