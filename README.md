...
> **首个 Alpha 已发布**：工程、配方、资源与关键玩法回归均已落盘；平衡数值仍可能在后续 Alpha 中调整。全部设计规格见 [`docs/design/`](docs/design/)。
>
...
**零电起步链**（猫须探测器 → 手摇发电机 → 电动马达 → 微电网）：

- **猫须探测器**：纯净方铅矿矿石 + 红合金线，ULV 元件线起点；
- **手摇发电机**：摇 400 tick（约 20 秒）≈ 满功率运行 600 tick（约 30 秒），曲柄可拆装（兼容 AE2 曲柄）；
- **ULV 构件线**：电动马达（对齐上游：铜线圈 + 磁化铁杆）、电动活塞、传送带（2 件/t）、电动泵（16 mB/t）、机械臂（2 件/周期）与流体调节器（16 mB/t）；**极化机**提供磁化铁杆的电力磁化路线（对照红石手工磁化）；
- **ULV 线材轧机 / 切割机 / 卷板机 / 车床**：锭→线、杆→螺栓、锭→板（+50% 产能 vs 锻锤）、螺栓→螺丝与木材车削——上游机器的配方子集下沉。

**微电网与储能**：

- **温差发电机**：贴热产电 1–5 EU/t 免维护保底；
- **红石发电机**：第一台可自动化燃料发电机（红石粉 1,200 EU）；
- **铅酸电池线**：单格电池 4k / 电池组 16k / 铅酸蓄电墙 24k EU，填平 1k → 80k 断档。
- **ULV 微型燃气轮机**：烧天然气/含硫气体/甲烷/含硫石脑油，8 EU/t · 1A——石油线副产与钻井天然气就此发电。

**电化学与石油**：

- **原型电解槽**：水电解与 PbO₂ 阳极氧化，ULV 电化学第一课；
- **铅室法制酸装置**：无电多方块，蒸汽产硫酸（1850 年代工业复刻）；
- **ULV 化学反应釜 / 流体固化器 / 流体提取机**：酸链、聚乙烯、橡胶链与成型固化；
- **石油线**：流体钻井机抽取基岩油田 → 原始蒸馏塔（3×3 可重复塔身，最高 6 层）→ 原始裂化机（乙烯）→ 聚乙烯，为 LV 时代储备塑料与燃料。

设计原则：**ULV 机器总能耗与上游一致**（慢速释放同额电力，LV 升级的收益是功率与速度而非省电）；仅下放 LV 级别配方，不引入并行/超频放大。
...
.\gradlew.bat runGameTestServer # 运行全部自动验收测试
.\gradlew.bat clean build       # 构建并检查发布 JAR（build/libs/）
